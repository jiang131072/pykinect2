...
class PyKinectRuntime(object):
    ...
    def __init__(self, frame_source_types):
        ...
        if self.frame_source_types & FrameSourceTypes_Color:
            ...
            self._color_frame_data_type = (
                ctypes.c_ubyte * self._color_frame_data_capacity.value
            )
            ...
            self._color_frame_arrived_event = (
                self._color_frame_reader.SubscribeFrameArrived()
            )
        ...
        if self.frame_source_types & FrameSourceTypes_Infrared:
            ...
            self._infrared_frame_data_type = (
                ctypes.c_ushort * self._infrared_frame_data_capacity.value
            )
            self._infrared_frame_data = ctypes.cast(
                self._infrared_frame_data_type(), ctypes.POINTER(ctypes.c_ushort)
            )
            self._infrared_frame_reader = self._infrared_source.OpenReader()
            self._infrared_frame_arrived_event = (
                self._infrared_frame_reader.SubscribeFrameArrived()
            )
        ...
        if self.frame_source_types & FrameSourceTypes_Depth:
            ...
            self._depth_frame_data_type = (
                ctypes.c_ushort * self._depth_frame_data_capacity.value
            )
            ...
            self._depth_frame_arrived_event = (
                self._depth_frame_reader.SubscribeFrameArrived()
            )
        ...
        if self.frame_source_types & FrameSourceTypes_BodyIndex:
            ...
            self._body_index_frame_data_type = (
                ctypes.c_ubyte * self._body_index_frame_data_capacity.value
            )
            self._body_index_frame_data = ctypes.cast(
                self._body_index_frame_data_type(), ctypes.POINTER(ctypes.c_ubyte)
            )
            self._body_index_frame_reader = self._body_index_source.OpenReader()
            self._body_index_frame_arrived_event = (
                self._body_index_frame_reader.SubscribeFrameArrived()
            )
        ...
        if self.frame_source_types & FrameSourceTypes_Body:
            ...
            self._body_frame_data_type = (
                ctypes.POINTER(IBody) * self._body_frame_data_capacity.value
            )
            self._body_frame_data = ctypes.cast(
                self._body_frame_data_type(), ctypes.POINTER(ctypes.POINTER(IBody))
            )
            self._body_frame_reader = self._body_source.OpenReader()
            self._body_frame_arrived_event = (
                self._body_frame_reader.SubscribeFrameArrived()
            )
        ...
        self._last_body_index_frame_access = self._last_body_index_frame_time = (
            start_clock
        )
        ...
        self._last_long_exposure_infrared_frame_access = (
            self._last_long_exposure_infrared_frame_time
        ) = start_clock
    ...
    def surface_as_array(self, surface_buffer_interface):
        ...
        self._PyObject_AsWriteBuffer(
            surface_buffer_interface, ctypes.byref(address), ctypes.byref(size)
        )
    ...
    def kinect_frame_thread(self):
        while 1:
            ...
            if self._handles[wait] == self._color_frame_arrived_event:
                ...
                self._handles[wait] == self._long_exposure_infrared_frame_arrived_event
            ):
    ...
    def handle_color_arrived(self, handle_index):
        colorFrameEventData = self._color_frame_reader.GetFrameArrivedEventData(
    ...
    def handle_depth_arrived(self, handle_index):
        depthFrameEventData = self._depth_frame_reader.GetFrameArrivedEventData(
            self._handles[handle_index]
        )
            ...
            try:
                with self._depth_frame_lock:
                    depthFrame.CopyFrameDataToArray(
                        self._depth_frame_data_capacity, self._depth_frame_data
                    )
    ...
    def handle_body_arrived(self, handle_index):
        bodyFrameEventData = self._body_frame_reader.GetFrameArrivedEventData(
            self._handles[handle_index]
        )
        ...
        try:
            ...
            try:
                with self._body_frame_lock:
                    bodyFrame.GetAndRefreshBodyData(
                        self._body_frame_data_capacity, self._body_frame_data
                    )
                    self._body_frame_bodies = KinectBodyFrameData(
                        bodyFrame, self._body_frame_data, self.max_body_count
                    )
    ...
    def handle_body_index_arrived(self, handle_index):
        bodyIndexFrameEventData = (
            self._body_index_frame_reader.GetFrameArrivedEventData(
                self._handles[handle_index]
            )
        )
            ...
            try:
                with self._body_index_frame_lock:
                    bodyIndexFrame.CopyFrameDataToArray(
                        self._body_index_frame_data_capacity,
                        self._body_index_frame_data,
                    )
    ...
    def handle_infrared_arrived(self, handle_index):
        infraredFrameEventData = self._infrared_frame_reader.GetFrameArrivedEventData(
            self._handles[handle_index]
        )
            ...
            try:
                with self._infrared_frame_lock:
                    infraredFrame.CopyFrameDataToArray(
                        self._infrared_frame_data_capacity, self._infrared_frame_data
                    )
