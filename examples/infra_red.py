...
class InfraRedRuntime(object):
    def __init__(self):
        ...
        self._kinect = runtime.PyKinectRuntime(com.FrameSourceTypes_Infrared)
    ...
    def draw_infrared_frame(self, frame, target_surface):
        if (
            frame is None
        ):  # some usb hub do not provide the infrared image. it works with Kinect studio though
            return
        target_surface.lock()
        f8 = np.uint8(frame.clip(1, 4000) / 16.0)
